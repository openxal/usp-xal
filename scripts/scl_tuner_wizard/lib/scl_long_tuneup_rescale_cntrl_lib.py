...
class NEW_Amp_and_Phases_to_EPICS_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		rescale_quads_button = JButton("Rescale SCL and HEBT1 quads")
		rescale_quads_button.addActionListener(Rescale_Quads_Listener(self.scl_long_tuneup_controller))
		restore_quads_button = JButton("Restore SCL and HEBT1 quads")
...
class Rescale_Quads_Listener(ActionListener):
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		...
		quad_ind = len(quads)-1
		for cav_wrapper in cav_wrappers:
			if(cav_wrapper.isGood):
				cav_pos = accSeq.getPosition(cav_wrapper.cav)
				eKin_out_init = cav_wrapper.eKin_out
				eKin_out_new  = cav_wrapper.rescaleBacket.eKin_out
				coeff = 1.0
				if(eKin_out_new > 0. and eKin_out_init > 0.):
					p_init = math.sqrt(eKin_out_init*(eKin_out_init+2*mass))
					p_new = math.sqrt(eKin_out_new*(eKin_out_new+2*mass))
					coeff = p_new/p_init
				while(accSeq.getPosition(quads[quad_ind]) > cav_pos):
					quad = quads[quad_ind]
					field = quad_field_dict[quad]
					quad.setField(coeff*field)
					#print "debug quad=",quad.getId()," Ginit=",field," Gnew=",coeff*field
					quad_ind -= 1
					if(quad_ind < 0):
						quad_ind += 1
						break
