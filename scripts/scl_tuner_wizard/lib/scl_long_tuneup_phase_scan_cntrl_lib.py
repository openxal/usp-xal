...
from scl_phase_scan_data_cleanup_lib import SCL_Scan_Data_CleanUp_Controller
...
class PhaseScan_Runner(Runnable):
	...
	def run(self):
		...
		for cav_table_ind in range(start_ind,last_ind+1):
			...
			if(cav_table_ind != 0):
				...
				if(self.fake_scan):
					cav_wrapper.initLiveAmp = cav_wrapper.cav.getDfltCavAmp()
				else:
					cav_wrapper.initLiveAmp = cav_wrapper.cav.getCavAmpSetPoint()
					ampSetGoal = cav_wrapper.getLiveAmlitudeSetGoal()
					if(math.fabs(ampSetGoal-cav_wrapper.initLiveAmp) > cav_amp_diff_threshold):
						messageTextField.setText("Cavity "+ cav_wrapper.alias + " has the amplitude different from the goal! Please check it!")
						scan_status_text.setText("Scan stopped. Cav. amp. is different from goal. Please check!")
						beamTrigger.setFakeScan(false)
						return
			if(cav_table_ind != 0):
				# ?????? should be tested with CA
			...
			while(repeat_count < repeat_count_max):
				...
				if(cav_table_ind != 0):
					...
					if(not res_phase_set):
						repeat_count += 1
						cav_wrapper.clean()
						scan_status_text.setText("Scan was bad! Count="+str(repeat_count)+" out of "+str(repeat_count_max)+ " Cavity="+cav_wrapper.alias)
						time.sleep(time_wait)	
						scan_success = false
				else:
					res_all_cavs_off = self.allCavitiesOffAnalysis(cav_wrapper)
					time.sleep(time_wait)
					if(not res_all_cavs_off):
						repeat_count += 1
						cav_wrapper.clean()
						scan_status_text.setText("Scan AllCavsOff was bad! Count="+str(repeat_count)+" out of "+str(repeat_count_max)+ " Cavity="+cav_wrapper.alias)
						time.sleep(time_wait)	
						scan_success = false
				#---- repeat scan - may be this time it will be better
				time.sleep(time_wait)
				if(scan_success == true): break				
			#----end of repeat_count loop
	...
	def setUpCavityPhase(self,cav_wrapper):
		...
		if(bad_points_count > 0):
			err = self.harmonicsAnalyzer.analyzeData(gd)
			harm_function = self.harmonicsAnalyzer.getHrmonicsFunction()
		#---- check that err is less than the maximal limit - otherwise the scan is bad
		err_max = scl_long_tuneup_phase_scan_controller.set_phase_shift_panel.max_avg_phase_err_text.getValue()
		if(abs(err) >  err_max):
			return false
	...
	def allCavitiesOffAnalysis(self,cav_wrapper):
		if(cav_wrapper != self.scl_long_tuneup_controller.cav0_wrapper): return false
		#--- wrap all BPMs phase scans at once
		if(self.wrapPhases): 
			res_phase_wrap = self.allBPMs_PhaseWrapper(cav_wrapper)	
			if(not res_phase_wrap): return false
		#---- also use only BPMs with minimal amplitudes more than bpm_amp_limit
		scl_long_tuneup_phase_scan_controller = self.scl_long_tuneup_controller.scl_long_tuneup_phase_scan_controller
		err_max_limit = scl_long_tuneup_phase_scan_controller.set_phase_shift_panel.max_avg_phase_err_text.getValue()		
		bpm_amp_limit = scl_long_tuneup_phase_scan_controller.post_scan_panel.amp_limit_text.getValue()		
		bpm_wrappers_avgPhase_err_arr = []
		for bpm_wrapper_ind in range(len(cav_wrapper.bpm_wrappers)):
			bpm_wrapper = cav_wrapper.bpm_wrappers[bpm_wrapper_ind]
			if(cav_wrapper.bpm_amp_phase_dict.has_key(bpm_wrapper)):
						(graphDataAmp,graphDataPhase) = cav_wrapper.bpm_amp_phase_dict[bpm_wrapper]
						nPoints = graphDataAmp.getNumbOfPoints()
						#print "debug bpm=",bpm_wrapper.alias," nPoints=",nPoints
						if(nPoints <= 0): return false
						min_amp = graphDataAmp.getY(0)
						for ind in range(nPoints):
							bpm_amp = graphDataAmp.getY(ind)
							if(bpm_amp < min_amp):
								min_amp = bpm_amp
						if(min_amp > bpm_amp_limit):
							bpm_wrappers_avgPhase_err_arr.append([bpm_wrapper,0.,0.])
		#-----check that err is less than the maximal limit - otherwise the scan is bad
		err_avg = 0.
		for bpm_ind in range(len(bpm_wrappers_avgPhase_err_arr)):
			[bpm_wrapper,avgPhase,err] = bpm_wrappers_avgPhase_err_arr[bpm_ind]
			(graphDataAmp,graphDataPhase) = cav_wrapper.bpm_amp_phase_dict[bpm_wrapper]
			avgPhase = 0.
			for ind in range(graphDataPhase.getNumbOfPoints()):
				avgPhase += graphDataPhase.getY(ind)
			avgPhase /= graphDataPhase.getNumbOfPoints()
			err2 = 0.
			for ind in range(graphDataPhase.getNumbOfPoints()):
				err2 += (avgPhase - graphDataPhase.getY(ind))**2
			err = math.sqrt(err2/graphDataPhase.getNumbOfPoints())
			err_avg += err
			if(err > err_max_limit):
				cav_wrapper.phase_scan_harm_err = err
				return false
		if(len(bpm_wrappers_avgPhase_err_arr) > 0):
			err_avg /= len(bpm_wrappers_avgPhase_err_arr)
		cav_wrapper.phase_scan_harm_err = err_avg
		return true
		...
		for ind in cav_selected_inds:
			...
			if(cav_wrapper == self.scl_long_tuneup_controller.cav0_wrapper):
				...
				for bpm_wrapper in cav_wrapper.bpm_wrappers:
					if(bpm_wrapper.isGood): break
				(graphDataAmp,graphDataPhase) = cav_wrapper.getAmpPhaseGraphs(bpm_wrapper)
				if(graphDataPhase != null):
					self.gp_bpm_phaseDiff.addGraphData(graphDataPhase)
			else:
...
class SetPhaseShiftAndTimeStep_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		self.phase_shift_text = DoubleInputTextField(-18.0,DecimalFormat("###.##"),6)
		...
		self.time_wait_text = DoubleInputTextField(0.5,DecimalFormat("##.###"),6)
		sub1_panel.add(time_wait_lbl)
		sub1_panel.add(self.time_wait_text)
		#------ Adaptive Feed Forward Settings
		aff_set_off_button = JButton("Set AFF Off")
		aff_set_on_button = JButton("Set AFF On")
		aff_reset_button = JButton("Reset AFF")
		aff_set_off_button.addActionListener(SetAFF_Off_Button_Listener(self.scl_long_tuneup_controller))
		aff_set_on_button.addActionListener(SetAFF_On_Button_Listener(self.scl_long_tuneup_controller))
		aff_reset_button.addActionListener(ResetAFF_Button_Listener(self.scl_long_tuneup_controller))
		sub1_panel.add(JLabel("--",JLabel.LEFT))
		sub1_panel.add(aff_set_off_button)
		sub1_panel.add(aff_set_on_button)
		sub1_panel.add(aff_reset_button)
		#------ maximal avg. phase error after the scan
		max_avg_phase_err_lbl = JLabel("<html>&delta;A<SUB>&phi;</SUB><html>",JLabel.RIGHT)
		self.max_avg_phase_err_text = DoubleInputTextField(4.0,DecimalFormat("##.#"),5)
		sub1_panel.add(JLabel("-- Max ",JLabel.LEFT))
		sub1_panel.add(max_avg_phase_err_lbl)
		sub1_panel.add(JLabel(" [deg]= ",JLabel.LEFT))
		sub1_panel.add(self.max_avg_phase_err_text)
		#------------------------------------------
...
class StartStopPhaseScan_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		self.phase_step_text = DoubleInputTextField(20.0,DecimalFormat("###.##"),8)
...
class PostPhaseScanActions_Panel(JPanel):
	def __init__(self,scl_long_tuneup_controller):
		...
		self.amp_limit_text = DoubleInputTextField(1.0,DecimalFormat("###.###"),8)
		...
		self.perform_clean_up_button = JRadioButton(" Do CleanUp ")
		self.perform_clean_up_button.setSelected(true)
		amp_limit_panel.add(amp_limit_lbl)
		amp_limit_panel.add(self.amp_limit_text)
		amp_limit_panel.add(amp_limit_bpm_button)
		amp_limit_panel.add(self.perform_clean_up_button)
		phase_point_del_panel = JPanel(FlowLayout(FlowLayout.LEFT,3,3))
		titled_border = BorderFactory.createTitledBorder(etched_border,"Edit the scan data")
		phase_point_del_panel.setBorder(titled_border)
		remove_phase_point_button = JButton(" Remove One Scan Phase Point ")
		remove_phase_point_button.addActionListener(Remove_Phase_Point_Button_Listener(self.scl_long_tuneup_controller))
		phase_point_del_panel.add(remove_phase_point_button)
		#--------------------------
		self.add(remove_bpm_panel)
		self.add(amp_limit_panel)
		self.add(phase_point_del_panel)
	...
	def actionPerformed(self,actionEvent):
		...
		for cav_wrapper in cav_wrappers:
			...
			for bpm_ind in range(len(bpm_wrappers)-1):
				...
				#--- update all bpm phases
				graphDataPhase1.updateValuesY(y_arr)
		#---- Possible CleanUp using the methods from the scl_phase_scan_data_cleanup panel
		cleanUp = scl_long_tuneup_phase_scan_controller.post_scan_panel.perform_clean_up_button.isSelected()
		if(cleanUp):
			scl_scan_data_cleanup_controller = scl_long_tuneup_phase_scan_controller.scl_scan_data_cleanup_controller
			scl_scan_data_cleanup_controller.analyze_Data_Method()
			scl_scan_data_cleanup_controller.cleanUp_Worst_Phase_Points_Data_Method()
			scl_scan_data_cleanup_controller.cleanUp_All_Bad_BPMs_Method()
...
class Remove_Phase_Point_Button_Listener(ActionListener):
	#set Adaptive Feed Forward in OFF state for all cavities
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		self.scl_long_tuneup_controller.getMessageTextField().setText("")
		scl_long_tuneup_phase_scan_controller = self.scl_long_tuneup_controller.scl_long_tuneup_phase_scan_controller
		bpm_phaseDiff_graph_panel = scl_long_tuneup_phase_scan_controller.bpm_phaseDiff_graph_panel
		gp_bpm_phaseDiff = bpm_phaseDiff_graph_panel.gp_bpm_phaseDiff
		cavs_table = scl_long_tuneup_phase_scan_controller.cavs_table
		cav_selected_inds = cavs_table.getSelectedRows()
		if(len(cav_selected_inds) != 1):
			self.scl_long_tuneup_controller.getMessageTextField().setText("Select only one cavity! Cannot remove phase point!")
			return
		if(cav_selected_inds[0] < 0): return
		cav_wrapper = null
		gd = null
		if(cav_selected_inds[0] == 0):
			cav_wrapper = self.scl_long_tuneup_controller.cav0_wrapper
			bpm_wrapper = null
			for bpm_wrapper in cav_wrapper.bpm_wrappers:
				if(bpm_wrapper.isGood): break
			...
			cav_wrapper = self.scl_long_tuneup_controller.cav_wrappers[cav_selected_inds[0]-1]
			gd = cav_wrapper.phaseDiffPlot
		#-------------------------------------------------------------
		gdVect = Vector()
		gdVect.add(gd)
		phaseMin = gp_bpm_phaseDiff.getCurrentMinX()
		phaseMax = gp_bpm_phaseDiff.getCurrentMaxX()
		bpm_phaseMin = gp_bpm_phaseDiff.getCurrentMinY()
		bpm_phaseMax = gp_bpm_phaseDiff.getCurrentMaxY()		
		resVect = GraphDataOperations.getGraphDataAndPointIndexInside(gdVect,phaseMin,phaseMax,bpm_phaseMin,bpm_phaseMax)
		(resGd,indInt) = resVect
		if(resGd == null or indInt == null):
			self.scl_long_tuneup_controller.getMessageTextField().setText("Cannot remove phase point! Zoom to have only one point on the left plot panel!")
			return
		point_ind = int(indInt)
		if(gd == cav_wrapper.phaseDiffPlot):
			gd.removePoint(point_ind)
		for bpm_wrapper in cav_wrapper.bpm_wrappers:
			(graphDataAmp,graphDataPhase) = cav_wrapper.getAmpPhaseGraphs(bpm_wrapper)
			if(graphDataAmp != null):
				graphDataAmp.removePoint(point_ind)
			if(graphDataPhase != null):
				graphDataPhase.removePoint(point_ind)
		#print "debug indInt=",int(indInt)
		
class SetAFF_Off_Button_Listener(ActionListener):
	#set Adaptive Feed Forward in OFF state for all cavities
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
	...
	def actionPerformed(self,actionEvent):
		for cav_wrapper in self.scl_long_tuneup_controller.cav_wrappers:
			if(cav_wrapper.isGood): cav_wrapper.setAFF_Off()

class SetAFF_On_Button_Listener(ActionListener):
	#set Adaptive Feed Forward in ON state for all cavities
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		
	def actionPerformed(self,actionEvent):
		for cav_wrapper in self.scl_long_tuneup_controller.cav_wrappers:
			if(cav_wrapper.isGood): cav_wrapper.setAFF_On()

class ResetAFF_Button_Listener(ActionListener):
	#reset Adaptive Feed Forward memory for all cavities
	def __init__(self,scl_long_tuneup_controller):
		self.scl_long_tuneup_controller = scl_long_tuneup_controller
		
	def actionPerformed(self,actionEvent):
		for cav_wrapper in self.scl_long_tuneup_controller.cav_wrappers:
			if(cav_wrapper.isGood): cav_wrapper.resetAFF()	
...
class SCL_Long_TuneUp_PhaseScan_Controller:
	def __init__(self,scl_long_tuneup_controller):
		...
		self.scl_scan_data_cleanup_controller = SCL_Scan_Data_CleanUp_Controller(self.scl_long_tuneup_controller)
		...
		graph_panel = JPanel(GridLayout(1,2))
		graph_panel.add(self.bpm_phaseDiff_graph_panel)		
		graph_panel.add(self.bpm_phase_and_amp_graph_panel)
		self.scans_clean_up_panel = JPanel(BorderLayout())
		self.scans_clean_up_panel.add(self.scl_scan_data_cleanup_controller.main_panel,BorderLayout.CENTER)
		graph_tabbed_panel = JTabbedPane(JTabbedPane.RIGHT)
		graph_tab_txt = "Plots"
		scans_clean_up_txt = "CleanUp"
		graph_tabbed_panel.addTab(graph_tab_txt,graph_panel)		
		graph_tabbed_panel.addTab(scans_clean_up_txt,self.scans_clean_up_panel)	
		#--------center panel = graphs + tables-------------
		center_panel = JPanel(GridLayout(2,1))
		center_panel.add(cavs_scan_panel)
		center_panel.add(graph_tabbed_panel)
