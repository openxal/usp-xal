...
class FULL_Scan_Fitter:
	...
	def fit(self):
		...
		cav_wrapper.cav_phase_unshifted_guess = cav_wrapper.cav_phase_guess
...
class Scan_Main_Panel(JPanel):
	def __init__(self,main_loop_controller,cav_controller):
		...
		self.full_scan_phase_step_text = DoubleInputTextField(10.0,DecimalFormat("###.##"),6)
		time_step_label = JLabel(" sleep time[sec]=",JLabel.RIGHT)
		self.time_step_text = DoubleInputTextField(1.9,DecimalFormat("###.##"),6)
		full_scan_fit_time_label = JLabel(" Fit time[sec]=",JLabel.RIGHT)
		self.full_scan_fit_time_text = DoubleInputTextField(10.,DecimalFormat("###.##"),6)
		...
		self.use_cav_off_text =  DoubleInputTextField(0.0,DecimalFormat("###.#"),5)
		...
		self.start_fit_phase_text = DoubleInputTextField(0.0,DecimalFormat("###.##"),8)
		stop_fit_phase_label = JLabel(" to phase [deg]=",JLabel.RIGHT)
		self.stop_fit_phase_text = DoubleInputTextField(0.0,DecimalFormat("###.##"),8)
...
class Cavity_BPMs_Controller:
	...
	def checkLastDataForBPM_SineData(self):
		...
		for bpm_scan_data in self.sin_wave_scan_data_arr:
			...
			if(not res0):
				txt += bpm_scan_data.bpm_wrapper.alias+" "
			res = res0 and res
		if(not res):
			for bpm_scan_data in self.sin_wave_scan_data_arr:
	...
	def runSetUpAlgorithm(self):
		...
		while(cav_phase <= 180.):
			if(miss_count > miss_count_max):
				...
				return (false,"Cannot get valid BPM data for 360 deg scan! Stop. Cavity="+self.cav_wrapper.alias+ " bad BPMs: "+bpm_txt)
			...
			(bpm_check_res,bpm_txt) = self.cav_bpms_controller.checkLastDataForBPM_SineData()
			if(not bpm_check_res):
