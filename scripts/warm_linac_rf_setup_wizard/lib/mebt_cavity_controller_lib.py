...
class Scan_Main_Panel(JPanel):
	def __init__(self,main_loop_controller,cav_controller):
		...
		self.full_scan_phase_step_text = DoubleInputTextField(20.0,DecimalFormat("###.##"),6)
		time_step_label = JLabel(" sleep time[sec]=",JLabel.RIGHT)
		self.time_step_text = DoubleInputTextField(1.5,DecimalFormat("###.##"),6)
		...
		self.scan_phase_step_text = DoubleInputTextField(2.0,DecimalFormat("###.##"),6)
		...
		self.bpm_iter_avg_text = DoubleInputTextField(5,DecimalFormat("##.#"),4)
		bpm_iter_fitting_steps_label = JLabel("   steps=",JLabel.RIGHT)
		self.bpm_iter_steps_text = DoubleInputTextField(2,DecimalFormat("##.#"),4)
...
class Local_BPMs_Table_Panel(JPanel):
	def __init__(self,main_loop_controller,cav_controller):
		...
		self.sine_min_acc_phase_text =  DoubleInputTextField(0.0,DecimalFormat("####.###"),10)
		sine_max_acc_phase_label = JLabel("Max. acc. phase[deg]=",JLabel.RIGHT)
		self.sine_max_acc_phase_text =  DoubleInputTextField(0.0,DecimalFormat("####.###"),10)
		zero_acc_phase_label = JLabel("Zero acc. phase[deg]=",JLabel.RIGHT)
		self.zero_acc_phase_text =  DoubleInputTextField(0.0,DecimalFormat("####.###"),10)
		zero_acc_phase_err_label = JLabel("Err. Zero acc. phase[deg]=",JLabel.RIGHT)
		self.zero_acc_phase_err_text =  DoubleInputTextField(0.0,DecimalFormat("####.###"),10)
		cav_energy_gain_label = JLabel("Cavity's E0TL [keV]=",JLabel.RIGHT)
		self.cav_energy_gain_text =  DoubleInputTextField(0.0,DecimalFormat("###.######"),10)
		...
		dump_bpm_data_button = JButton("Dump Sin-Like BPM data")
		dump_bpm_data_button.addActionListener(Dump_SinLike_BPM_Data_Button_Listener(self.main_loop_controller,self.cav_controller))
		...
		#tmp_panel5.add(dump_bpm_data_button)
...
class Dump_SinLike_BPM_Data_Button_Listener(ActionListener):
	def __init__(self,main_loop_controller,cav_controller):
		self.main_loop_controller = main_loop_controller
		self.cav_controller = cav_controller
		
	def actionPerformed(self,actionEvent):
		graphs_panel = self.cav_controller.graphs_panel
		local_bpms_table_panel = self.cav_controller.local_bpms_table_panel
		cav_zero_accel_phase = local_bpms_table_panel.zero_acc_phase_text.getValue()
		cav_bpms_controller = self.cav_controller.cav_bpms_controller
		bpm_scan_data = cav_bpms_controller.getBPM_ScanData_for_SineWave()
		phase_gd = bpm_scan_data.phase_gd
		amp_gd = bpm_scan_data.amp_gd
		fl_out = open("sin_like_phase_bpm_amp.dat","w")
		for ind in range (phase_gd.getNumbOfPoints()):
			phase = phase_gd.getX(ind)
			phase = makePhaseNear( (phase - cav_zero_accel_phase) - 90., 0.)
			bpm_amp = amp_gd.getY(ind)
			print "phase=",phase," amp=",bpm_amp
			st = " %10.4f  %10.4f "%(phase,bpm_amp)
			fl_out.write(st+"\n")
		fl_out.close()
...
class MEBT_Cavity_Controller(Abstract_Cavity_Controller):
	...
	def findGuessCavParametersAfterSineLikeScan(self):	
		...
		self.cav_wrapper.cav_phase_unshifted_guess = makePhaseNear(cav_zero_accel_phase,0.)
		self.cav_wrapper.cav_amp_coeff = self.cav_wrapper.design_amp/self.cav_wrapper.initAmp
		self.cav_wrapper.cav_amp_coeff_unshifted_guess = self.cav_wrapper.cav_amp_coeff
