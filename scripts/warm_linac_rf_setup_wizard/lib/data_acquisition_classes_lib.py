...
class BPM_Scan_Data:
	...
	def clean(self):
		...
		self.xAvg_gd.removeAllPoints()	
		self.yAvg_gd.removeAllPoints()
	...
	def addPoint(self,cav_phase):
		...
		xAvg = self.bpm_wrapper.bpm.getXAvg()
		yAvg = self.bpm_wrapper.bpm.getYAvg()
		self.addExternalPoint(cav_phase,bpm_amp,bpm_phase,xAvg,yAvg)
	...
	def addExternalPoint(self,cav_phase,bpm_amp,bpm_phase,xAvg = 0.,yAvg = 0.):
		...
		self.xAvg_gd.addPoint(cav_phase,xAvg)
		self.yAvg_gd.addPoint(cav_phase,yAvg)
	...
	def setCavAmplitudeParam(self,cav_amp):
		...
		self.xAvg_gd.setGraphProperty(GRAPH_LEGEND_KEY,self.bpm_wrapper.alias+" CavAmp= %6.4f "%self.cav_amp)
		self.yAvg_gd.setGraphProperty(GRAPH_LEGEND_KEY,self.bpm_wrapper.alias+" CavAmp= %6.4f "%self.cav_amp)
	...
	def removeLastPoint(self):
		...
		self.xAvg_gd.removePoint(nP-1)
		self.yAvg_gd.removePoint(nP-1)
	...
	def writeDataToXML(self,root_da):
		...
		dumpGraphDataToDA(self.xAvg_gd,bpm_scan_data_da,"xAvg_gd")
		dumpGraphDataToDA(self.yAvg_gd,bpm_scan_data_da,"yAvg_gd")
	...
	def readDataFromXML(self,bpm_scan_data_da):
		...
		readGraphDataFromDA(self.xAvg_gd,bpm_scan_data_da,"xAvg_gd")
		readGraphDataFromDA(self.yAvg_gd,bpm_scan_data_da,"yAvg_gd")
...
class FaradayCup_Scan_Data:
	def __init__(self,dtl_acceptance_scans_controller,dtl_acc_scan_cavity_controller):
		...
		self.fc_scan_gd.setGraphProperty(GRAPH_LEGEND_KEY,self.cav_wrapper.alias)
	...
	def addExternalPoint(self,cav_phase,fc_charge):
		if(self.fc_scan_gd.getNumbOfPoints() != 0):
			cav_phase_old = self.fc_scan_gd.getX(self.fc_scan_gd.getNumbOfPoints()-1)
			cav_phase = makePhaseNear(cav_phase,cav_phase_old)
	...
	def setCavAmplitudeParam(self,cav_amp):
		self.cav_amp = cav_amp
		self.fc_scan_gd.setGraphProperty(GRAPH_LEGEND_KEY,self.cav_wrapper.alias+" CavAmp= %6.4f "%self.cav_amp)
	...
	def checkLastDataPoint(self,min_charge):
		...
		if(nP > 1):
			if(math.fabs(self.amp_gd.getY(nP-1)) > min_charge):
				if(math.fabs(self.fc_scan_gd.getY(nP-2) - self.fc_scan_gd.getY(nP-1)) < 0.000000001):
					return false
		return res
	...
	def writeDataToXML(self,root_da):
		...
		dumpGraphDataToDA(self.fc_scan_gd,fc_scan_data_da,"fc_acceptance_scan_gd")
	...
	def readDataFromXML(self,fc_scan_data_da):
		...
		readGraphDataFromDA(self.fc_scan_gd,fc_scan_data_da,"fc_acceptance_scan_gd")
