...
class PASTA_Fitting_Diff_Calculator:
	...
	def caluclateDiff(self,nPhasePoints,Ekin_in,cav_amp_coeff,phase_shift):
		...
		#----- variable BPMs phase shift : DTL2 BPMs are not synchronized
		bpm_phase_diff_offset_param = self.cav_controller.bpm_phase_diff_offset_param
		#--------------------------------------------------
		...
		for [cav_amp,bpm_scan_data0,bpm_scan_data1] in pasta_scan_data_arr:
			...
			for ind in range(bpm_scan_data0.phase_gd.getNumbOfPoints()):
				...
				#----- variable BPMs phase shift : DTL2 BPMs are not synchronized
				diff_fit += bpm_phase_diff_offset_param
				#----------------------------------------------------------------
				diff = makePhaseNear(diff_fit - diff,0.)
				phase_diff2 += diff**2
				if(diff**2 > max_dev):
					max_dev = diff**2
...
class PASTA_Fitter:
	...
	def fit(self):
		...
		#----- variable BPMs phase shift : DTL2 BPMs are not synchronized
		var = Variable("bpms_phase_shift",bpm_phase_diff_offset_param, - Double.MAX_VALUE,  Double.MAX_VALUE)
		variables.add(var)
		variables_fit.add(var)
		delta_hint.addInitialDelta(var,5.0)		
		...
		[phase_shift,Ekin_in,cav_amp_coeff, bpm_phase_diff_offset_param] = scorer.getTrialParams(trial)
		cav_wrapper.Ekin_in = Ekin_in
		cav_wrapper.cav_phase_guess = makePhaseNear(cav_wrapper.design_phase + phase_shift,0.)
		cav_wrapper.cav_phase_unshifted_guess = makePhaseNear(cav_wrapper.cav_phase_guess,0.)
		...
		cav_wrapper.cav_amp_coeff_unshifted_guess = cav_amp_coeff	
		cav_wrapper.newAmp = cav_wrapper.design_amp/cav_amp_coeff
		cav_wrapper.avg_pasta_err = math.sqrt(err2)
		self.cav_controller.bpm_phase_diff_offset_param = bpm_phase_diff_offset_param
		#------------------------------------------
		self.solver = null
		#------------------------------------------		
		nPhasePoints = 30
		...
		cav_wrapper.newAmp = cav_wrapper.design_amp/cav_amp_coeff
		cav_wrapper.newPhase = cav_wrapper.cav_phase_guess + self.cav_controller.guess_phase_shift_text.getValue()	
		return (true,"")
...
class PASTA_Scorer(Scorer):
	...
	def __init__(self,main_loop_controller,cav_controller,variables):
		...
		self.bpm_phase_diff_offset_param = self.variables.get(3).getInitialValue()
	...
	def getDiff2(self):
		...
		self.cav_controller.bpm_phase_diff_offset_param = self.bpm_phase_diff_offset_param
		(res,txt,phase_diff2) = self.cav_controller.pasta_fitting_diff_calculator.caluclateDiff(nPhasePoints,Ekin_in,cav_amp_coeff,phase_shift)
		if(not res):
			return 0.
	...
	def getTrialParams(self,trial):
		...
		if(var_map.containsKey(variables.get(3))): self.bpm_phase_diff_offset_param = trial.getTrialPoint().getValue(variables.get(3))
		return [self.phase_shift,self.Ekin_in,self.cav_amp_coeff, self.bpm_phase_diff_offset_param]
...
class Scan_Main_Panel(JPanel):
	def __init__(self,main_loop_controller,cav_controller):
		...
		self.full_scan_phase_step_text = DoubleInputTextField(15.0,DecimalFormat("###.##"),6)
		time_step_label = JLabel(" sleep time[sec]=",JLabel.RIGHT)
		self.time_step_text = DoubleInputTextField(2.2,DecimalFormat("###.##"),6)
		...
		self.min_cav_amp_text =  DoubleInputTextField(0.0,DecimalFormat("##.#####"),8)
		max_cav_amp_label = JLabel("  Max=",JLabel.RIGHT)
		self.max_cav_amp_text =  DoubleInputTextField(0.0,DecimalFormat("##.#####"),8)
		step_cav_amp_label = JLabel(" N amp. scan steps=",JLabel.RIGHT)
		self.nsteps_cav_amp_text =  DoubleInputTextField(2.0,DecimalFormat("###.#"),5)
		live_cav_amp_label = JLabel("   Live Ampl.=",JLabel.RIGHT)
		self.live_cav_amp_text =  DoubleInputTextField(0.0,DecimalFormat("##.#####"),8)
		...
		self.scan_phase_width_text = DoubleInputTextField(30.0,DecimalFormat("###.##"),6)
		scan_phase_step_label = JLabel("  Phase step[deg]=",JLabel.RIGHT)
		self.scan_phase_step_text = DoubleInputTextField(3.0,DecimalFormat("###.##"),6)
		min_bpm_amp_label = JLabel("    Min BPM ampitude[mA]=",JLabel.RIGHT)
		self.min_bpm_amp_text = DoubleInputTextField(1.0,DecimalFormat("###.##"),6)
		...
		self.start_fit_phase_text = DoubleInputTextField(0.0,DecimalFormat("###.##"),6)
		stop_fit_phase_label = JLabel(" to phase [deg]=",JLabel.RIGHT)
		self.stop_fit_phase_text = DoubleInputTextField(0.0,DecimalFormat("###.##"),6)
		n_points_fit_label = JLabel(" fit points=",JLabel.RIGHT)
		self.n_points_fit_text = DoubleInputTextField(10.0,DecimalFormat("###.#"),6)
		time_limit_fit_label = JLabel(" Fit time[sec]=",JLabel.RIGHT)
		self.time_limit_fit_text = DoubleInputTextField(20.0,DecimalFormat("###.#"),6)
	...
	def updateGraphs(self):
		...
		for [cav_amp,bpm_scan_data0,bpm_scan_data1] in cav_bpms_controller.pasta_scan_data_arr:
			...
			for ind in range(bpm_scan_data0.phase_fit_gd.getNumbOfPoints()):
				...
				y_diff = makePhaseNear(y1-y0 + bpm_phase_diff_offset_param,phase_diff_gd.getValueY(x))
...
class Local_BPMs_and_Params_Tables_Panel(JPanel):
	def __init__(self,main_loop_controller,cav_controller):
		...
		self.fitting_params_table.setPreferredScrollableViewportSize(Dimension(350,80))
		...
		self.pasta_error_text = DoubleInputTextField(0.0,DecimalFormat("####.######"),12)
...
class Fit_Params_Table_Model(AbstractTableModel):
	...
	def getValueAt(self,row,col):
		if(col == 0):
			...
			if(row == 3): return "BPMs Phase Diff. Offset"
		cav_wrapper = self.cav_controller.cav_wrapper
		if(col == 1):
			...
			if(row == 3): return "%6.2f"%	self.cav_controller.bpm_phase_diff_offset_param
		if(col == 2):
	...
	def setValueAt(self, value, row, col):
		...
		if(col == 1):
			...
			if(row == 3): self.cav_controller.bpm_phase_diff_offset_param = float(value)
		if(col == 2):
...
class DTL1_Cavity_Controller(Abstract_Cavity_Controller):
	def __init__(self,main_loop_controller,cav_wrapper):
		...
		#---- introduce phase offset from data: BPMs are not synchronized
		self.bpm_phase_diff_offset_param = 0.	
		#----------------------------------------------------------------
		...
		#---- no fitting for Ekin_in for DTL1 for now: BPMs are not synchronized
		self.cav_wrapper.Ekin_in_fit_use = False
	...
	def guessInitialPASTA_Params(self):
		...
		#---------------------------------------------------------------------------------------
		#---- This part (Ekin_in_guess) does not work now: BPMs in DTL2 are not synchronized
		#---- the bpm diff. phase shift is -12 deg on +20 keV
		Ekin_delta = - 0.02*(bpm_diff_phase_avg - bpm_diff_phase_design)/12.0
		Ekin_delta = 0.
		self.bpm_phase_diff_offset_param = bpm_diff_phase_avg - bpm_diff_phase_design
	...
	def writeDataToXML(self,root_da):
		...
		cav_cntrl_params_da.setValue("bpms_phase_diff_offset",self.bpm_phase_diff_offset_param)
		#---------------------------------------------------------
	...
	def readDataFromXML(self,cav_cntrl_data_da):	
		...
		if(params_da.hasAttribute("bpms_phase_diff_offset")):
			self.bpm_phase_diff_offset_param = params_da.doubleValue("bpms_phase_diff_offset")
		#-------------------------------------------------
