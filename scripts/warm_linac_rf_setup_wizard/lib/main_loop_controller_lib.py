...
class Cavity_Wrapper:
	def __init__(self,cav,pos,alias,amp_pv_name,phase_pv_name,cav_pwr_pv_name):
		...
		#---- unshifted guess values of amplitude coef. and phase
		self.cav_phase_unshifted_guess = 0.
		self.cav_amp_coeff_unshifted_guess = 1.
	...
	def writeDataToXML(self,cav_wrapper_da):
		...
		design_and_fit_params_da.setValue("unshifted_phase_guess",self.cav_phase_unshifted_guess)
		design_and_fit_params_da.setValue("unshifted_amp_coeff_guess",self.cav_amp_coeff_unshifted_guess)
	...
	def readDataFromXML(self,cav_wrapper_da):
		...
		if(design_and_fit_params_da.hasAttribute("unshifted_phase_guess")):
			self.cav_phase_unshifted_guess = design_and_fit_params_da.doubleValue("unshifted_phase_guess")
		if(design_and_fit_params_da.hasAttribute("unshifted_amp_coeff_guess")):
			self.cav_amp_coeff_unshifted_guess = design_and_fit_params_da.doubleValue("unshifted_amp_coeff_guess")
...
class Start_Stop_Panel(JPanel):
	def __init__(self,main_loop_controller):
		...
		correct_phase_shifts_button = JButton("Correct Phase Shifts for 360 deg Scans for Slct. Cavs")
