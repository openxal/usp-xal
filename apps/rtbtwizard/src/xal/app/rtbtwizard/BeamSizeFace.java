...
public class BeamSizeFace extends JPanel{
    ...
    //These correction values come from J.Holmes' July 8, 2016 Tech Note
    //These values replace a hardcoded 'magic number' correction. 2017-01-10  - nhe
    private double alphaPBW = 0.62;// Alpha for the proton beam window. Aluminum=0.62, Inconel=0.86 
    private double xPencilScatterRMS = 7.71; //RMS size of pencil beam on target with beam window scattering. Al=7.71, Inconel=14.39 
    private double yPencilScatterRMS = 7.70; //RMS size of pencil beam on target with beam window scattering. Al=7.70, Inconel=14.41 
...
//        PhaseMatrix matRef = (probe).stateResponse(currentElem, refStart);
    ...
    public void resetResultsTable(){
        ...
        // Beam sizes with scattering correction.
        double xScatCorr = xPencilScatterRMS*xPencilScatterRMS*alphaPBW;// Original Empirical Correction was 11.0
        double yScatCorr = yPencilScatterRMS*yPencilScatterRMS*alphaPBW;// Original Empirical Correction was 11.0
		double xtargetwin = Math.sqrt(rx*rx + xScatCorr);
		double ytargetwin = Math.sqrt(ry*ry + yScatCorr);
