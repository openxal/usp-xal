...
public class VADocument extends AcceleratorDocument implements ActionListener, PutListener {
	...
	private java.util.List<WireScanner> wss;
...
//	private RecentFileTracker _probeFileTracker;
	...
	public void makeMainWindow() {
		...
		/** add digaplot */
		final FunctionGraphsJPanel beamdispplot = new FunctionGraphsJPanel();
		final FunctionGraphsJPanel sigamplot = new FunctionGraphsJPanel();
		_diagplot = new DiagPlot(beamdispplot, sigamplot);
		
		/** Create panl with plots */
		final JPanel beamPlotsPanel =  (JPanel) _windowReference.getView("BeamPlots");
		beamPlotsPanel.setLayout(new GridLayout(2,1));
		final JPanel lowPanel = new JPanel(new BorderLayout());
		lowPanel.add(sigamplot,BorderLayout.CENTER);		
		beamPlotsPanel.add(beamdispplot);
		beamPlotsPanel.add(lowPanel);
...
//			EnvelopeProbeState state = (EnvelopeProbeState) stateIter.next();
			...
			double sigmaz= twiss[2].getEnvelopeRadius();
			double sigmax= twiss[0].getEnvelopeRadius() * 1000;
			double sigmay= twiss[1].getEnvelopeRadius() * 1000;
			
			double c_light = 299792458.0;
			double sigma_z_deg = 360.*402.5e+6*sigmaz/(c_light*state.getBeta());
			
			tempbeampos.add(position);
			tempbeamx.add(x);
			tempbeamy.add(y);
			tempsigmaz.add(sigma_z_deg);
			tempsigmax.add(sigmax);
			tempsigmay.add(sigmay);
		}	        		
		...
		double beamsigmax[]=new double[tempbeampos.size()];
		double beamsigmay[]=new double[tempbeampos.size()];
		...
		for (i = 0; i < tempbeampos.size(); i++) {
			...
			beamsigmax[i]=tempsigmax.get(i);
			beamsigmay[i]=tempsigmay.get(i);
		}
		try {
			_diagplot.showbeampositionplot(beamp, beamx, beamy);
			_diagplot.showsigmazplot(beamp, beamsigmaz);
			_diagplot.showsigmaXYplot(beamp, beamsigmax, beamsigmay);
...
//                final PhaseVector coordinates = cmpCalcEngine.computeCoordinatePosition(probeState);
				...
				//BPM's amplitude calculations, BPM frequency in MHz
				double bpm_freq = bpm.getBPMBucket().getFrequency();
				double c_light = 299792458.0;
				final Twiss[] twiss = cmpCalcEngine.computeTwissParameters(probeState);
				double sigma_z = 2*Math.PI*bpm_freq*1.0e+6*twiss[2].getEnvelopeRadius()/(c_light*probeState.getBeta());
				double bpm_amp = 0.2;
				if(Math.abs(sigma_z) < Math.PI){
				     bpm_amp = 20.0*Math.exp(-sigma_z*sigma_z/2.0);
				}
				//System.out.println("debug bpm="+bpm.getId()+" freq="+ bpm_freq+" size[deg]"+(sigma_z*180./Math.PI));
			
				// hardwired BPM amplitude 1% noise and zero offset respectively
				bpmAmpAvgChannel.putVal( NoiseGenerator.setValForPV(bpm_amp, 1., 0. ) );
				...
				if ( !( currentProbe instanceof TransferMapProbe ) && !( bpm instanceof RingBPM ) ) {
					...
					double bpm_phase = probeState.getTime() * 360. * ( ( (BPMBucket)bpm.getBucket("bpm") ).getFrequency() * 1.e6 ) % 360.0;
					bpmPhaseAvgChannel.putValCallback( bpm_phase + (Math.random()-0.5)*0.001, this );
				}
			} catch (ConnectionException e) {
				System.err.println( e.getMessage() );
			} catch (PutException e) {
		...
		// for WSs		
    for ( final WireScanner ws : wss ) {
			Channel wsX = ws.getChannel(WireScanner.HORIZONTAL_SIGMA_GAUSS_HANDLE);
			//System.out.println( "debug channel Channel wsX = "+wsX.channelName());
			Channel wsY = ws.getChannel(WireScanner.VERTICAL_SIGMA_GAUSS_HANDLE);
			try {
				...
				if (modelScenario.getProbe() instanceof EnvelopeProbe) {
					...
					tempWSp.add(getSelectedSequence().getPosition(ws));
		...
		/**showWSplot*/
		try {			
			_diagplot.show_ws_plot(wsp, wsx, wsy);
		} catch (ConnectionException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		} catch (GetException e1) {
	...
	public void selectedSequenceChanged() {
		...
		if (selectedSequence != null) {
			...
			wss = getSelectedSequence().getAllNodesWithQualifier( QualifierFactory.qualifierWithStatusAndType( true, "WS" ) );
...
class DiagPlot {
	...
	public void showsigmaXYplot(double[] p,double[] sigmax, double[] sigmay) throws ConnectionException, GetException {
		Datasigmax.updateValues(p, sigmax);	
		Datasigmay.updateValues(p, sigmay);	
	}
	
	public void show_ws_plot(double[] wsp,double[] wsx, double[] wsy) throws ConnectionException, GetException {		
	...
	public void setupPlot(FunctionGraphsJPanel beampositionplot,FunctionGraphsJPanel sigamplot) {
	    ...
	    DataWSx.setGraphProperty(_sigamplot.getLegendKeyString(), "WSx");		
	    DataWSy.setGraphProperty(_sigamplot.getLegendKeyString(), "WSy");
		Datasigmaz.setGraphProperty(_sigamplot.getLegendKeyString(), "sigma z");
		Datasigmax.setGraphProperty(_sigamplot.getLegendKeyString(), "sigma x");
		Datasigmay.setGraphProperty(_sigamplot.getLegendKeyString(), "sigma y");
		
		DataWSx.setDrawLinesOn(false);
		DataWSy.setDrawLinesOn(false);
		DataWSx.setGraphPointSize(9);
		DataWSy.setGraphPointSize(9);
	    ...
	    Datasigmaz.setDrawPointsOn(false);
	    Datasigmax.setDrawPointsOn(false);
	    Datasigmay.setDrawPointsOn(false);
	    
	    Datasigmaz.setLineThick(3);
	    Datasigmax.setLineThick(3);
	    Datasigmay.setLineThick(3);
